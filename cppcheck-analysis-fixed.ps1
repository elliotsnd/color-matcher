...
if (-not $cppcheckPath) {
    Write-Host "❌ Cppcheck not found! Installing via winget..." -ForegroundColor Red
    try {
        winget install --id=Cppcheck.Cppcheck -e
        Write-Host "✅ Cppcheck installed successfully!" -ForegroundColor Green
    } catch {
        Write-Host "❌ Failed to install Cppcheck. Please install manually from https://cppcheck.sourceforge.io/" -ForegroundColor Red
...
try {
    if ($OutputFormat -eq "xml") {
        $process = Start-Process -FilePath "cppcheck" -ArgumentList $cppcheckArgs -RedirectStandardError $true -RedirectStandardOutput $true -PassThru -Wait -NoNewWindow
        ...
        try {
            ...
            if ($errors) {
                ...
                foreach ($errorItem in $errors) {
                    ...
                    $file = Split-Path $errorItem.location.file -Leaf
...
Write-Host "  • Verbose output: .\cppcheck-analysis-fixed.ps1 -Verbose" -ForegroundColor White
Write-Host "  • Open HTML report: .\cppcheck-analysis-fixed.ps1 -OpenReport" -ForegroundColor White
Write-Host "  • JSON output: .\cppcheck-analysis-fixed.ps1 -OutputFormat json" -ForegroundColor White
